...
lobf_k = slope_k * company_stats['num_stores'] + intercept_k 
plt.scatter(company_stats['num_stores'], company_stats['avg_monthly_sales']) 
plt.plot(company_stats['num_stores'], lobf_k, color='red') 
...
plt.plot(plot_data['date'], plot_data['sum_sale_dollars']) 
...
for yr in sorted(plot_data['year'].unique()):
    ...
    plt.plot(yearly_data['month'], yearly_data['sum_sale_dollars'])
    plt.ylabel("Total Sale Dollars (1 Million $ Scale)")
    plt.xlabel("Month")
...
# Plot total sale dollars over time (Combined Years in one Graph)
